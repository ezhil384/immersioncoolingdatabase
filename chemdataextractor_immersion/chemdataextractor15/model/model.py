...
from .units.thermal_conductivity import ThermConductivityModel
from .units.viscosity import ViscosityModel
...
from ..parse.common import lbrct, rbrct
from ..parse.auto import AutoTableParser, AutoSentenceParser
from ..parse.apparatus import ApparatusParser


from ..parse.thermal_conductivity import ThermParser
from ..parse.viscosity import ViscosityParser
from ..parse.flash_point import FlashPointParser
from ..parse.dielectric_constant import DielectricConstantParser
...
class ThermalConductivity(ThermConductivityModel):
...
class Viscosity(ViscosityModel):
    value = StringType(contextual=False, required=True)
    units = StringType(contextual=False, required=True)
    specifier = StringType(contextual=False)
    compound = ModelType(Compound, contextual=False)
    parsers = [ViscosityParser()]


class FlashPoint(TemperatureModel):
    value = StringType(contextual=False, required=True)
    units = StringType(contextual=False, required=True)
    specifier = StringType(contextual=False)
    compound = ModelType(Compound, contextual=False)
    parsers = [FlashPointParser()]

class DielectricConstant(DimensionlessModel):
    """
    Model for capturing dielectric constant (relative permittivity).
    Dimensionless — no units element. Uses a custom parser to avoid
    false positives (e.g. DOI numbers) that the auto-parsers produced.
    """
    value = StringType(contextual=False, required=True)
    units = StringType(contextual=False, required=False)
    specifier = StringType(contextual=False)
    measurement_conditions = StringType(contextual=False, required=False)
    compound = ModelType(Compound, contextual=False)
    parsers = [DielectricConstantParser()]

class DielectricLabel(BaseModel):
    """
    Separate model for labeling/identifying dielectric specifiers 
    without associated quantities for testing purposes.
    """
    specifier_expression = (I('dielectric') + I('constant') | R(r'^ε_?r?$')).add_action(merge)
    label = StringType(parse_expression=specifier_expression)
    compound = ModelType(Compound, required=False)
    parsers = [AutoSentenceParser(), AutoTableParser()]