...
def value_element_plain():
    ...
    # New: Multiplier logic (handles 5 * 10 or 5 x 10)
    multiplier = R('^[×\*xX]$')
    scientific_notation = (number + multiplier + number).add_action(merge)
    
    # Update the single value to prioritize the multiplied format
    value_single = (Optional(R('^[~∼˜\<\>]$')) + Optional(R('^[\-–−]$')) + (scientific_notation | number))('raw_value').add_action(merge)
    joined_range = R('^[\+\-–−]?\d+(\.\d+)?[\-–−~∼˜]\d+(\.\d+)?$')('raw_value').add_action(merge)
...
def extract_conduct_value(string):
    ...
    if ',' in string or 'and' in string:
        string_list = re.split('and|,', string)
    else:
        string_list = [string]
    extracted_values = []
    for jj in string_list:
        if re.findall(r'(\d+.)?\d+ × 10 [−−−-] \d', jj):
            jlist = extract_value(jj)
            try:
                extracted_value = jlist[-3] * jlist[-2] ** -jlist[-1]
            except BaseException:
                return False
        elif re.findall(r'10 [−−−-] \d', jj):
            jlist = extract_value(jj)
            extracted_value = jlist[-2] ** -jlist[-1]
        elif re.findall(r'(\d+.)?\d+ × 10\d', jj):
            jlist = extract_value(jj)
            try:
                extracted_value = jlist[-2] * 10 ** (jlist[-1] - 100)
            except BaseException:
                return False
        elif re.findall('^10[1-9]$', jj):
            jlist = extract_value(jj)
            extracted_value = 10 ** (jlist[-1] - 100)
        else:
            extracted_value = extract_value(jj)
