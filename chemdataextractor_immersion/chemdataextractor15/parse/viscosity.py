# -*- coding: utf-8 -*-
"""
chemdataextractor.parse.battery_viscosity.py
~~~~~~~~~~~~~~~~~~~~~~~~~~~

Parser for viscosity.
...
visc = (power | range | value | ordinal)('value')
...
visc_specifier = (Optional(I('dynamic') | I('kinematic')) + (I('viscosity') | I('viscosities')))('specifier')
...
visc_and_units = (
    ...
    visc +
    units +
    Optional(rbrct).hide())('visc')

visc_specifier_and_value = Optional(prefix) + (Optional(delim).hide() + Optional(lbrct | I('[')).hide() + visc + units + Optional(rbrct | I(']')).hide())('visc')
...
prefix_cem_value = (
    ...
    Optional(lbrct).hide() +
    (visc_specifier_and_value | visc_and_units) +
    Optional(rbrct).hide())('visc_phrase')
...
cem_prefix_value = (
    ...
    + Optional(lbrct).hide() + (visc_specifier_and_value | visc_and_units) + Optional(rbrct).hide()
...
prefix_value_cem = (
    ...
    SkipTo(visc_specifier_and_value | visc_and_units) +
    (visc_specifier_and_value | visc_and_units) +
    Optional(
    ...
    Optional(visc_specifier_and_value | visc_and_units) +
...
value_prefix_cem = (Optional(I('of')) +
                    ...
                    (multi_cem | cem_prefix | lenient_chemical_label))('visc_phrase')
...
cem_value_prefix = ((multi_cem | cem_prefix | lenient_chemical_label)
                    ...
                    + Optional(I('its') | I('their') | I('the')).hide() + visc_specifier)('visc_phrase')
...
class ViscosityParser(BaseSentenceParser):
    """"""
    root = bc

    def interpret(self, result, start, end):
        compound = self.model.fields['compound'].model_class()
        raw_value = first(result.xpath('./visc/value/text()'))
        raw_units = first(result.xpath('./visc/units/text()'))
        try:
            specifier = ' '.join(
                [i for i in (first(result.xpath('./specifier'))).itertext()])
        except BaseException:
            specifier = ''
        battery_viscosity = self.model(raw_value=raw_value,
                                       raw_units=raw_units,
                                       specifier=specifier,
        ...
        for cem_el in result.xpath('./cem'):
            ...
            battery_viscosity.compound = compound
            battery_viscosity.compound.names = cem_lists
            battery_viscosity.compound.labels = cem_el.xpath('./labels/text()')
            log.debug(battery_viscosity.serialize())
        yield battery_viscosity
